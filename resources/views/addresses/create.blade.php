                    ...
                    <form action="{{ route('addresses.store') }}" method="POST">
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" name="address" id="address" class="w-full p-3 border rounded-md" value="{{ old('address') }}" required>
                            </div>

                            <div>
                                <label for="city" class="block text-gray-700 dark:text-gray-200">{{ __('Ciudad') }}</label>
                                <input type="text" name="city" id="city" class="w-full p-3 border rounded-md" value="{{ old('city') }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="phone_number" id="phone_number" class="w-full p-3 border rounded-md" value="{{ old('phone_number') }}" required>
                            </div>

                            <div>
                                <label for="is_default" class="inline-block text-gray-700 dark:text-gray-200">{{ __('Establecer como predeterminada') }}</label>
                                <!-- Campo oculto para is_default = 0 cuando no esté marcado -->
                                <input type="hidden" name="is_default" value="0">
                                <input type="checkbox" name="is_default" id="is_default" value="1" {{ old('is_default') ? 'checked' : '' }}>
                            </div>

                            <div class="mt-6">
                                <button type="submit" class="bg-primary text-white px-4 py-2 rounded-md">{{ __('Guardar Dirección') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
